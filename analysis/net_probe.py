...
def probe_ws_book_lag(seconds: float = 35.0, out: str = "") -> None:
    """How fresh the Polymarket market WS actually is, from THIS box.

    pmengine currently gets its executable book from the REST poller at
    PMENGINE_BOOK_POLL_MS (live: 2000) walked serially per token, because a
    code comment says the market websocket is geoblocked from US IPs. That
    claim is testable, and the whole book-age line of the latency budget
    hangs on it, so it gets tested rather than inherited.

    Subscribes to the tokens of whichever windows the engine most recently
    wrote to its own book tape -- real, live, actively-quoted markets -- and
    measures (exchange event timestamp -> local receipt) per event type.

    Caveat on the absolute level: the exchange stamps with ITS clock and we
    receive on OURS, so the median carries whatever host clock offset exists.
    The spread (p50 to p99) and the inter-update cadence do not, and they are
    the parts the argument actually rests on.
    """
    try:
        ...
        print(f"\n(ws book-lag probe skipped: {e})")
        return

    tape = os.path.expanduser("~/.pmt/engine/book-tape.jsonl")
        ...
        with open(tape) as f:
            for line in f:
                try:
                    ...
                    continue
                if d.get("ev") == "book":
                    slugs[d.get("slug", "")] = d.get("t", 0)
    # Only windows that have not settled yet. A slug whose window closed still
    # sits at the top of the tape by recency, but its book is dead -- it emits
    # a stale snapshot and nothing else, which reads as a 3-second-old feed and
    # is entirely an artefact of asking a settled market for quotes.
    now = time.time()
    fresh = []
    for s, t in sorted(slugs.items(), key=lambda kv: -kv[1]):
        parts = s.split("-")
        if len(parts) != 4 or not parts[3].isdigit() or not parts[2].endswith("m"):
            continue
        end = int(parts[3]) + int(parts[2][:-1]) * 60
        if end > now + 30:          # at least 30s of life left to observe
            fresh.append(s)
        if len(fresh) >= 3:
            break
    if not fresh:
        # The tape only has windows the engine armed; if it is stopped, or the
        # fleet is disarmed, every slug on it is settled. The series are on a
        # fixed clock, so the currently-open window is computable without the
        # tape at all.
        start = int(now // 300) * 300
        fresh = [f"{sym}-updown-5m-{start}" for sym in ("btc", "eth", "sol")]
        print(f"  (tape has no unsettled window — using the clock-derived live "
              f"5m windows at {start})")
    live = fresh
    toks: list[str] = []
    for slug in live:
        try:
            r = requests.get("https://gamma-api.polymarket.com/markets",
                             params={"slug": slug}, timeout=8).json()
            if r:
                toks += json.loads(r[0]["clobTokenIds"])
        except Exception:  # noqa: BLE001
            continue
    if not toks:
        print("\n(ws book-lag probe skipped: no live tokens resolved)")
        return

    async def run():
        lags: dict[str, list[float]] = {"book": [], "price_change": [],
                                        "last_trade_price": []}
        gaps: list[float] = []
        seen: dict[str, float] = {}
        n = 0
        ...
        async with websockets.connect(
            ...
            await ws.send(json.dumps({"assets_ids": toks[:6], "type": "market"}))
            end = time.time() + seconds
            first = None
            while time.time() < end:
                try:
                    m = await asyncio.wait_for(ws.recv(),
                ...
                for it in (d if isinstance(d, list) else [d]):
                    ...
                    tstamp = it.get("timestamp")
                    if tstamp and et in lags:
                        lags[et].append(recv - float(tstamp))
                    aid = it.get("asset_id")
                    if aid:
                        if aid in seen:
                            gaps.append(recv - seen[aid])
                        seen[aid] = recv
        return connect_ms, first, n, lags, gaps

    connect_ms, first, n, lags, gaps = asyncio.run(run())
    print()
    print("=" * 96)
    print("POLYMARKET MARKET WEBSOCKET — is it usable from here, and how fresh?")
    print("=" * 96)
    print(f"  tokens {len(toks[:6])} from live windows {live}")
    print(f"  handshake {connect_ms:.0f}ms   first message +{(first or 0):.0f}ms   "
          f"{n:,} events in {seconds:.0f}s ({n/seconds:.0f}/s)")
    print()
    print(f"  {'event type':<20} {'n':>7} {'p10':>8} {'p50':>8} {'p90':>8} "
          f"{'p99':>8} {'min':>8}   (event ts -> local receipt, ms)")
    for k, v in lags.items():
        if v:
            print(f"  {k:<20} {len(v):7} {q(v,.1):8.1f} {q(v,.5):8.1f} "
                  f"{q(v,.9):8.1f} {q(v,.99):8.1f} {min(v):8.1f}")
    if gaps:
        print()
        print(f"  per-token inter-update gap: p50 {q(gaps,.5):.0f}ms  "
              f"p90 {q(gaps,.9):.0f}ms  p99 {q(gaps,.99):.0f}ms  max {max(gaps):.0f}ms")
    bl = lags.get("book") or lags.get("price_change") or []
    if bl:
        print(f"""
  COMPARE: the engine's live book comes from the REST poller at 2000ms, walked
  serially per token. This feed is p50 {q(bl,.5):.0f}ms old at receipt and updates a given
  token every ~{q(gaps,.5) if gaps else 0:.0f}ms. The book-age line of the latency budget is not a
  network limit; it is a datasource choice.""")
    if out:
        with open(out, "w") as f:
            json.dump({"lags": lags, "gaps": gaps, "n": n,
                       "seconds": seconds, "tokens": toks[:6]}, f)


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--ws-lag", action="store_true",
                    help="only measure the Polymarket market WS event->receipt lag")
    ap.add_argument("--ws-seconds", type=float, default=35.0)
    ...
    if args.ws_lag:
        probe_ws_book_lag(args.ws_seconds,
                          args.out.replace(".json", "-wslag.json") if args.out else "")
        return 0
    if args.ab_only:
