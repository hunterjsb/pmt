...
PMT_FLEET_NODE=             # optional; this box's name on fleet-wide views
PMT_FLEET_WALLETS=          # optional; label=0xaddr pairs for OTHER engines'
                            # wallets — read-only, display only. See "Watch
                            # charts row" below.
```
...
- **Watch charts row** (`pmtrader/watch_charts.py` render + math, `pmtrader/watch_feeds.py` corpus tails; `pmt crypto watch`, `--no-charts` to drop it): one full-width braille chart panel between the risk header and the windows table, drawn off data the fetch worker already holds — **zero new calls in the render path**, which is the rule the whole dashboard is split in two for (`docs/LESSONS.md#L28`). **Per cared window** (cared_windows: stakes first, then only the shortest-duration idle arms — the 15m measurement canaries stay in the "+N more armed" note until one fills): a compact sparkline of the underlying against that window's own settlement strike (the TWAP print at `start-60`, keyed exactly as `rtds_read.twap_marks` keys it; the axis always contains the strike, so distance from that rail IS the margin in bp, and the side we hold rides the identity glyph and colour), plus — tall mode — an INTENT row: gate state, the engine's own reason string verbatim, the market's de-vigged P(up) off the book tape, and the dollars committed. **The echo triangle** under them, for the window closest to settling: rtds / spot venue / odds overlaid on one 30s axis (each z-scaled to its own std — probability and dollars share an axis by shape only); an `echo` row measuring the pairwise delta-correlation-maximising lag over 180s of 1s deltas (positive = left leads; `echo_lag` returns nothing below ECHO_MIN_N overlapping deltas rather than a confident r off noise — and both series carry SOURCE timestamps, so this measures the oracle's information lag, not our relay); and a `lat` row with each clock's age and sample rate, because a lag read off a stale feed is a lie with a decimal point. The odds series is the engine's own `book-tape.jsonl` tailed like the other corpora — de-vigged up-mid, shortest-duration window per symbol. A cumulative-P&L sparkline panel and an OHLC candle form both lived on this row briefly and were cut (flat lines with one cliff; unreadable at half-block resolution) — sparklines + stats is the settled form. **Fleet money is a header row instead** (`watch_ui.fleet_row`): `fleet 24h` — this box plus every peer wallet, each summed over the trailing day (safe to sum: under the series partition two accounts never hold the same window; riding windows are not P&L; a peer whose ledger never landed is skipped, never shown as zero). **Peers are named, not discovered**: `PMT_FLEET_WALLETS=eu=0x…` (label=address pairs; `PMT_FLEET_NODE` names this box), each walked by the SAME `_tape_scoreboard` grader on the worker's 60s lane, because the fleet runs one *account* per engine and this box's rows contain nothing about another's. A peer walk is floored at 24h ON PURPOSE — `wallet.activity_since` reaches for this box's activity dump below `IMMUTABLE_AFTER_S`, and splicing one account's fresh rows onto another's dump would invent a ledger, so `_tape_scoreboard` refuses a deeper peer floor outright. The corpora are tailed INCREMENTALLY from a remembered byte offset (rtds alone is ~240MB/day; a re-read per poll is the blocking-call-in-the-loop shape `analysis/watch_load.md` is about), with day rollover, truncation and a capped per-poll chunk all handled. Everything drops rather than degrades: a screen too short gets compact charts, then no charts, before the tape loses its floor; a missing spot corpus loses the lead block whole; a torn corpus line is `errlog.note`'d and skipped.
