...
- **Watch charts row** (`pmtrader/watch_charts.py` render + math, `pmtrader/watch_feeds.py` corpus tails; `pmt crypto watch`, `--no-charts` to drop it): one full-width braille chart panel between the risk header and the windows table, drawn off data the fetch worker already holds — **zero new calls in the render path**, which is the rule the whole dashboard is split in two for (`docs/LESSONS.md#L28`). **feeds**: per armed window, the underlying against that window's own settlement strike (the TWAP print at `start-60`, keyed exactly as `rtds_read.twap_marks` keys it, so the chart's zero line is the reference the arm was priced against); the axis always contains the strike, so distance from that rail IS the margin in bp, and the side we hold rides the identity glyph and the colour. Under it the settlement stream and a leading spot venue over the same 30s at the same scale, each centred on its own mean — the horizontal offset between the two lines is the ~3s maker lead, shown rather than asserted. Drawn TALL (two terminal rows per chart, eight braille levels) when the screen affords it, compact when it does not; a cumulative-P&L sparkline panel used to share this row and earned nothing at braille resolution, so it was cut and the whole row is price now. **Fleet money is a header row instead** (`watch_ui.fleet_row`): `fleet 24h` — this box plus every peer wallet, each summed over the trailing day (safe to sum: under the series partition two accounts never hold the same window; riding windows are not P&L; a peer whose ledger never landed is skipped, never shown as zero). **Peers are named, not discovered**: `PMT_FLEET_WALLETS=eu=0x…` (label=address pairs; `PMT_FLEET_NODE` names this box), each walked by the SAME `_tape_scoreboard` grader on the worker's 60s lane, because the fleet runs one *account* per engine and this box's rows contain nothing about another's. A peer walk is floored at 24h ON PURPOSE — `wallet.activity_since` reaches for this box's activity dump below `IMMUTABLE_AFTER_S`, and splicing one account's fresh rows onto another's dump would invent a ledger, so `_tape_scoreboard` refuses a deeper peer floor outright. The corpora are tailed INCREMENTALLY from a remembered byte offset (rtds alone is ~240MB/day; a re-read per poll is the blocking-call-in-the-loop shape `analysis/watch_load.md` is about), with day rollover, truncation and a capped per-poll chunk all handled. Everything drops rather than degrades: a screen too short gets compact charts, then no charts, before the tape loses its floor; a missing spot corpus loses the lead block whole; a torn corpus line is `errlog.note`'d and skipped.
