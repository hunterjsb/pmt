...
- **Watch charts row** (`pmtrader/watch_charts.py` render + math, `pmtrader/watch_feeds.py` corpus tails; `pmt crypto watch`, `--no-charts` to drop it): two braille line charts between the risk header and the windows table, both drawn off data the fetch worker already holds — **zero new calls in the render path**, which is the rule the whole dashboard is split in two for (`docs/LESSONS.md#L28`). **P&L**: cumulative WALLET-GRADED P&L over the trailing 24h, one line per engine — this box off the same `eff_windows` the header's ledger is folded from, each peer wallet off its own `_tape_scoreboard` walk on the 60s lane, and a fleet line that is their sum (safe to sum: under the series partition two accounts never hold the same window). Riding windows are NOT in it — an undecided position is not P&L — and an engine whose ledger we cannot reach gets no line rather than a flat one. **feeds**: per armed window, the underlying against that window's own settlement strike (the TWAP print at `start-60`, keyed exactly as `rtds_read.twap_marks` keys it, so the chart's zero line is the reference the arm was priced against); the axis always contains the strike, so distance from that rail IS the margin in bp, and the side we hold rides the identity glyph and the colour. Under it the settlement stream and a leading spot venue over the same 30s at the same scale, each centred on its own mean — the horizontal offset between the two lines is the ~3s maker lead, shown rather than asserted. **Peers are named, not discovered**: `PMT_FLEET_WALLETS=eu=0x…` (label=address pairs; `PMT_FLEET_NODE` names this box), because the fleet runs one *account* per engine and this box's rows contain nothing about another's. A peer walk is floored at 24h ON PURPOSE — `wallet.activity_since` reaches for this box's activity dump below `IMMUTABLE_AFTER_S`, and splicing one account's fresh rows onto another's dump would invent a ledger, so `_tape_scoreboard` refuses a deeper peer floor outright. The corpora are tailed INCREMENTALLY from a remembered byte offset (rtds alone is ~240MB/day; a re-read per poll is the blocking-call-in-the-loop shape `analysis/watch_load.md` is about), with day rollover, truncation and a capped per-poll chunk all handled. Everything drops rather than degrades: a screen too short loses the whole row before the tape loses its floor, a missing spot corpus loses the lead block whole, a torn corpus line is `errlog.note`'d and skipped.
