...
_ERA_REG = (cc.eras.Era("old", 0.0, "before"),
            cc.eras.Era("mid", 2_000_000.0, "middle"),
            cc.eras.Era("new", 3_000_000.0, "current"))
...
def test_score_activity_ceiling_selects_windows_by_start_not_row_time(monkeypatch):
    ...
    _install_fake_pipeline(monkeypatch, _era_rows((1_999_000, True), (2_000_100, False)),
                            {}, {})
    rows = cc.wallet.fetch_wallet_activity("0xabc", 0.0)

    sb = cc.score_activity(rows, 0.0, ceiling=2_000_000.0)
...
def test_score_activity_ceiling_is_half_open_at_the_boundary(monkeypatch):
    ...
    assert cc.score_activity(rows, 0.0, ceiling=2_000_000.0)["wins"] == 0
    assert cc.score_activity(rows, 2_000_000.0)["wins"] == 1
...
def test_era_scoreboards_file_each_window_by_its_start(monkeypatch):
    ...
    out = cc.era_scoreboards(rows, tape_records=[], registry=_ERA_REG)
...
def test_era_scoreboards_partition_the_all_time_scoreboard_exactly(monkeypatch):
    ...
    all_time = cc.score_activity(rows, 0.0)
    out = cc.era_scoreboards(rows, tape_records=[], registry=_ERA_REG)
...
def test_era_scoreboards_carry_each_eras_own_breakeven_bar(monkeypatch):
    ...
    out = {r["name"]: r for r in cc.era_scoreboards(rows, tape_records=[],
                                                     registry=_ERA_REG)}
    assert out["old"]["breakeven"] == pytest.approx(
        cc.effectiveness.breakeven_win_rate(out["old"]["sb"]["eff_windows"]))
    assert out["new"]["breakeven"] is None


def test_era_scoreboards_span_hours_are_none_for_the_open_left_era(monkeypatch):
    _install_fake_pipeline(monkeypatch, [], {}, {})
    out = {r["name"]: r for r in cc.era_scoreboards([], tape_records=[],
                                                     registry=_ERA_REG)}
...
def test_era_scoreboards_never_walk_the_wallet(monkeypatch):
    ...
    monkeypatch.setattr(cc.wallet, "fetch_wallet_activity", boom)
    monkeypatch.setattr(cc.wallet, "funder_address", boom)
    monkeypatch.setattr(cc.tape, "iter_records", lambda *a, **kw: iter(()))
    assert len(cc.era_scoreboards([], tape_records=[], registry=_ERA_REG)) == 3
...
def test_era_scoreboards_read_the_tape_once_not_once_per_era(monkeypatch):
    ...
    monkeypatch.setattr(cc.tape, "iter_records", counting_iter)
    monkeypatch.setattr(cc, "_gamma_resolution_cached", lambda slug: None)
    cc.era_scoreboards([], registry=_ERA_REG)
...
def test_score_activity_prefers_handed_in_tape_records(monkeypatch):
    ...
    monkeypatch.setattr(cc.tape, "iter_records", boom)
    monkeypatch.setattr(cc, "_gamma_resolution_cached", lambda slug: None)
    recs = [{"ev": "roll", "t": 5.0}, {"ev": "roll", "t": 50.0}]
    assert cc.score_activity([], 0.0, tape_records=recs)["rolls"] == 2
    assert cc.score_activity([], 0.0, ceiling=10.0, tape_records=recs)["rolls"] == 1
...
def _stats_cli(monkeypatch, args, rows):
    ...
    _install_fake_pipeline(monkeypatch, rows, {}, {})
    monkeypatch.setattr(cc, "_engine_post", down)
    monkeypatch.setattr(cc, "_api", down)
    return CliRunner().invoke(cc.crypto_stats, args)


def _real_era_rows():
    """One window inside each SHIPPED era, so the CLI test exercises the
    registry that actually ships rather than a stand-in."""
    return _era_rows(*[(int(e.start) + 300, True) for e in cc.eras.ERAS])
...
def test_stats_era_flag_scopes_the_report_to_one_era(monkeypatch):
    ...
    assert [e["name"] for e in out["eras"]] == cc.eras.names()
    assert sum(e["wins"] for e in out["eras"]) == len(cc.eras.ERAS)
...
def test_stats_default_view_carries_every_era(monkeypatch):
    ...
    assert out["wins"] == len(cc.eras.ERAS)          # all-time stays the headline
    assert [e["name"] for e in out["eras"]] == cc.eras.names()
